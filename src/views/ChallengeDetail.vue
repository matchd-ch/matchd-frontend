  ...
  <div v-if="challenge.data" class="challenge-detail flex flex-col min-h-content-with-fixed-bars">
    ...
    >
      ...
      <SelectPillGroup class="mb-4">
        <SelectPill v-for="keyword in challenge.data.keywords" :key="keyword.id" :has-input="false">
          {{ keyword.name }}
        </SelectPill>
      </SelectPillGroup>
    </PostingSection>
    ...
    <PostingSection v-if="isLoggedIn" title="Neugierig? Dann lassen wir es matchen!">
      <template v-if="challenge.data.company">
        <router-link :to="{ name: 'CompanyDetail', params: { slug: challenge.data.company.slug } }">
      ...
      <template v-else>
        ...
        >
          <h3 class="text-heading-sm flex">
            <template v-if="challenge.data.student?.firstName && challenge.data.student?.lastName">
