<template>
  <form v-if="profileData && softSkills.length && culturalFits.length" @submit="onSubmit">
    ...
    >
      <template #label>Ihrem Bildungsinstitut ist es wichtig, dass ...</template>
    </SelectPillMultiple>
    <template v-if="edit">
      <teleport to="footer">
...
const veeForm = useForm<UniversityProfileStep4Form>({});
...
const profileData = computed(() => {
  ...
  return universityProfileStep4FormMapper(user.value);
});
